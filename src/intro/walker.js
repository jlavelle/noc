...
const sketch = p => {
  ...
  const anyStep = () =>
    weightedChoice([...standardMoves, ...diagonalMoves, ...noMove])(
      Math.random()
    );

  // Exercise I.1
  const downRightStep = () =>
    weightedChoice(replaceWeights(standardMoves)([10, 10, 8, 8]))(
      Math.random()
    );
  ...
  const perlinStep = ({ walker, width, height, t }) => {
    ...
    return Vec2(f([0, w])(p.noise(t)) - walker.x)(
